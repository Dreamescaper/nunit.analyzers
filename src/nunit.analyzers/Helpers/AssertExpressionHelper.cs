...
namespace NUnit.Analyzers.Helpers
{
    internal static class AssertExpressionHelper
    {
        ...
        /// <summary>
        /// Returns 'actual' assertion argument expression. Supports both classic and constraint assertion model.
        /// </summary>
        public static ExpressionSyntax TryGetActualValue(InvocationExpressionSyntax assertExpression, IMethodSymbol methodSymbol)
        {
            return GetArgumentExpression(assertExpression, methodSymbol, NunitFrameworkConstants.NameOfActualParameter);
        }
        ...
        private static ExpressionSyntax GetArgumentExpression(InvocationExpressionSyntax invocationSyntax,
            IMethodSymbol methodSymbol, string argumentName)
        {
            var arguments = invocationSyntax.ArgumentList.Arguments;

            // Try find named argument
            var argument = arguments.FirstOrDefault(a => a.NameColon?.Name.Identifier.Text == argumentName);

            if (argument == null)
            {
                var methodParameter = methodSymbol.Parameters.FirstOrDefault(p => p.Name == argumentName);
